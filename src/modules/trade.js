    ...
    if (cmd[0] === '!trade' && typeof cmd[1] !== 'undefined' && ivy.con) {
        ...
        ivy.con.query('SELECT tu.nickname, tu.trainerCode FROM tradetags AS tt ' +
            ...
            if(results.length !== 0){
                ...
                ivy.con.query('SELECT nickname, trainerCode FROM tradeusers WHERE LOWER(nickname) LIKE ? OR LOWER(username) LIKE ? or userId LIKE ?', ['%'+tag+'%', '%'+tag+'%', '%'+tag+'%'], (err2, results2) => {
        ...
        if (typeof user !== 'undefined'){
            ...
            ivy.con.query('SELECT * FROM tradeusers WHERE userId = ?', [ user.id ], (err, results, fields) => {
                if (err){
                    console.log(err);
                ...
                if (results.length === 0){
                    ...
                    ivy.con.query('INSERT INTO tradeusers (trainerCode, username, nickname, userId) VALUES(?,?,?,?)', [ code, user.displayName, nick, user.id ], (err2) => {
                       if (err2){
                           console.log(err2);
                       ...
                       for (let i in roles){
                           for (let j in ivy.config.tradetags){
                               if (roles[i].name.toLowerCase() === ivy.config.tradetags[j].toLowerCase()){
                       ...
                       if (addRoles.length !== 0){
                           ...
                           ivy.con.query('INSERT INTO tradeusertags(tagId, userId) SELECT id as tagId, (SELECT id FROM tradeusers WHERE userId = "'+user.id+'") as userId FROM tradetags  WHERE name IN (' + '?,'.repeat(addRoles.length).slice(0, -1) + ')', addRoles, (err3, results2) => {
                               if (err3){
                                   console.log(err3);
                    ...
                    ivy.con.query('UPDATE tradeusers SET trainerCode = ?, nickname = ? WHERE userId = ?', [ code, user.displayName, user.id ], (err2) => {
                        if (err2){
                            console.log(err2);
